...
**This public repository contains fictional demonstration data only.** It does not contain résumés, email addresses, application history, OAuth tokens, browser sessions, cookies or production credentials.

## What it demonstrates

- A polished responsive Next.js and TypeScript interface
- Multi-source opportunity normalization
...
The public demo uses deterministic fictional data so reviewers get a fast, reliable experience. The production architecture keeps provider adapters server-side, normalizes them to one opportunity contract, and never exposes provider keys to the browser. See [Architecture](docs/ARCHITECTURE.md) and [API documentation](docs/API.md).

## 中文说明

这是一个隐私安全的 AI 外包机会工作台作品集，展示从多渠道机会发现、统一数据结构、匹配排序，到基于已验证经历生成申请方案的完整流程。

公开仓库只包含虚构演示数据，不包含真实姓名、邮箱、简历、投递历史、OAuth 令牌、浏览器 Cookie、会话或生产密钥。
