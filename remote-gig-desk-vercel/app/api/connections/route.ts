...
export async function POST(request:Request){
  ...
  if(auth.startsWith("Bearer ")){
    ...
    const taskRows=await sql`UPDATE applications SET lease_owner=${agent.id},lease_expires_at=${leaseUntil},attempt_count=attempt_count+1,last_error=CASE WHEN platform_key=${"proginn"} AND last_error LIKE ${"%application_url_missing%"} THEN ${"authorized_proginn_retry_consumed"} ELSE last_error END,updated_at=${now} WHERE id=(SELECT id FROM applications WHERE owner_email=${agent.ownerEmail} AND (status=${"queued_for_browser"} OR (status=${"verification_required"} AND platform_key=${"proginn"}) OR (status=${"submission_failed"} AND platform_key=${"proginn"} AND last_error=${"application_url_missing"} AND attempt_count=${18})) AND (lease_owner IS NULL OR lease_expires_at<${now}) ORDER BY created_at ASC LIMIT 1) RETURNING id,title,source_url AS "sourceUrl",application_url AS "applicationUrl",destination,platform_key AS "platformKey",status,application_letter AS "applicationLetter",proposed_rate AS "proposedRate",attempt_count AS "attemptCount",materials`;
