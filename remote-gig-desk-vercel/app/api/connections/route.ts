...
export async function POST(request:Request){
  ...
  if(auth.startsWith("Bearer ")){
    ...
    const proginnStates=await sql`SELECT status,delivery_state AS "deliveryState",last_error AS "lastError",attempt_count AS "attemptCount" FROM applications WHERE owner_email=${agent.ownerEmail} AND platform_key=${"proginn"} ORDER BY updated_at DESC LIMIT 5`;
    console.info("proginn_task_states",proginnStates);
    const taskRows=await sql`UPDATE applications SET lease_owner=${agent.id},lease_expires_at=${leaseUntil},attempt_count=attempt_count+1,last_error=CASE WHEN platform_key=${"proginn"} AND last_error LIKE ${"%application_url_missing%"} THEN ${"authorized_proginn_retry_consumed"} WHEN platform_key=${"proginn"} AND last_error=${"authorized_proginn_retry_consumed"} AND attempt_count=${20} THEN ${"authorized_proginn_retry_executing"} ELSE last_error END,updated_at=${now} WHERE id=(SELECT id FROM applications WHERE owner_email=${agent.ownerEmail} AND (status=${"queued_for_browser"} OR (status=${"verification_required"} AND platform_key=${"proginn"}) OR ((status=${"submission_failed"} AND platform_key=${"proginn"} AND last_error=${"application_url_missing"} AND attempt_count=${18}) OR (status=${"submission_failed"} AND platform_key=${"proginn"} AND last_error=${"authorized_proginn_retry_consumed"} AND attempt_count=${20}))) AND (lease_owner IS NULL OR lease_expires_at<${now}) ORDER BY created_at ASC LIMIT 1) RETURNING id,title,source_url AS "sourceUrl",application_url AS "applicationUrl",destination,platform_key AS "platformKey",status,application_letter AS "applicationLetter",proposed_rate AS "proposedRate",attempt_count AS "attemptCount",materials`;
    ...
    const tasks=(taskRows as any[]).map(task=>({...task,destination:task.destination||(task.platformKey==="proginn"?task.sourceUrl:""),attachments:Array.isArray(task.materials?.attachments)?task.materials.attachments.map((item:any)=>({...item,url:`https://remote-gig-desk-vercel.vercel.app/api/attachments?id=${encodeURIComponent(item.id)}`})):[],applicantProfile,execution:browserExecutionContract(task.applicationUrl||task.destination||(task.platformKey==="proginn"?task.sourceUrl:""))}));
