...
export async function GET(){
  ...
  try{
    ...
    const rows=await sql`SELECT count(*) FILTER (WHERE last_seen_at>=${cutoff})::int AS online,count(*) FILTER (WHERE last_seen_at>=${cutoff} AND version=${"0.4.0"})::int AS current,count(*) FILTER (WHERE last_seen_at>=${cutoff} AND (version IS NULL OR version<>${"0.4.0"}))::int AS outdated FROM browser_agents`;
