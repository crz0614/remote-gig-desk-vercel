...
export default function PortfolioBuilder(){
  ...
  const load=async()=>{
    ...
    if(profileResponse.ok){
      ...
      if(saved){
        ...
        if(merged.skills.trim()&&merged.experience.trim()&&merged.education.trim())setNotice("✓ 个人资料已恢复：技能、经历和教育信息均已加密保存，AI 会按岗位选择调用。");
      }
    }
  };
  useEffect(()=>{void load();},[]);
  ...
  const publish=async()=>{setPublishing(true);setNotice("正在同步 GitHub 并启用 Pages…");try{const r=await fetch("/api/portfolio/publish",{method:"POST"});const result=await r.json();if(r.ok){setNotice("作品集已同步 GitHub 并提交部署");await load();window.open(result.deploymentUrl,"_blank","noopener,noreferrer");}else setNotice(result.error==="github_authorization_required"?"请先在“连接”中授权 GitHub":`发布失败：${result.error||"请重试"}`);}catch{setNotice("发布请求失败，请稍后重试");}finally{setPublishing(false);}};
