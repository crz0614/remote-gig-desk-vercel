...
export default function PortfolioBuilder(){
  ...
  const publish=async()=>{setPublishing(true);setNotice("正在同步 GitHub 并启用 Pages…");const r=await fetch("/api/portfolio/publish",{method:"POST"});const result=await r.json();if(r.ok){setNotice("作品集已同步 GitHub 并提交部署");await load();window.open(result.deploymentUrl,"_blank","noopener,noreferrer");}else setNotice(result.error==="github_authorization_required"?"请先在“连接”中授权 GitHub":`发布失败：${result.error||"请重试"}`);setPublishing(false);};
