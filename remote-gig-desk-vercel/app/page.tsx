...
const applicationReplies:Reply[]=[
  {id:"acclaro",company:"Acclaro · Rhea Thapa",subject:"Freelance IT & Software Translator",date:"2026-06-23",status:"需要回复",tone:"action",summary:"招聘方感谢你的申请，并要求确认当前所在地以及你来自哪里。",translation:"你好，若竹：\n\n感谢你的申请。请确认你目前所在的位置，以及你来自哪里。\n\n此致，\nRhea Thapa\n高级人才招聘专员",original:"Hi Ruozhu,\n\nThanks for applying.\n\nCan you kindly confirm your current location and from where do you belong?\n\nRegards,\nRhea Thapa\nSenior Talent Finder",next:"建议回复：中国香港 / 英国曼彻斯特，并说明目前可在香港远程工作。发送前由我为你起草英文回复。",gmailUrl:"https://mail.google.com/mail/#all/19ef346fc2b5b1ce"},
  {id:"centific",company:"Centific / OneForma",subject:"Milky Way · Maps Evaluation Hong Kong",date:"2026-06-23",status:"需要完成申请",tone:"action",summary:"香港地图搜索结果评估兼职；常规报酬 10 美元/小时，完成带薪培训期间约 12 美元/小时。",translation:"工作内容：评估地图应用中的搜索查询、用户意图和数据准确性。\n\n要求：目前位于香港；英语熟练；能够理解英文指南并通过认证。\n\n报酬：常规约 10 美元/小时；两周带薪培训通过后，培训期约 12 美元/小时。\n\n下一步：注册 OneForma、申请 Milky Way、完成指南和模拟器认证以及安全审查。平台明确禁止使用 VPN 和 AI 代替考试。",original:"Hello, I am Karine from Centific Recruitment Team. Thank you for applying for Milky Way - Maps Evaluation - Hong Kong.\n\nProject rate: 10 USD/hour. Paid training is approximately 12 USD/hour for two weeks after successful completion. Applicants must currently be in Hong Kong, understand English guidelines, complete certifications, and must not use a VPN or AI during testing.",next:"该项目可以继续，但时薪偏低。若接受，需要按邮件步骤完成 OneForma 注册及两项认证；认证过程不能使用 AI。",gmailUrl:"https://mail.google.com/mail/#all/19ef2f9f33040c95"},
  {id:"proxy",company:"Black Sea Network · Proxy Infrastructure Engineer",subject:"Re: Application for Proxy Infrastructure Engineer",date:"2026-06-15",status:"待核验后联系",tone:"warning",summary:"对方同意进一步沟通，并要求转到 WhatsApp。发件人为个人 Gmail 地址，建议先核验公司与联系人身份。",translation:"好的，我们可以在 WhatsApp 上沟通。\n\n这是一条积极回复，但邮件未提供面试时间、岗位合同或公司域名邮箱，因此联系前应先核验身份，不发送证件、验证码或付款。",original:"OK, we can talk on WhatsApp → +852 59829107",next:"先用简短英文消息确认公司名称、岗位范围、合同形式和预算，再决定是否继续。",gmailUrl:"https://mail.google.com/mail/#all/19ecba50e81c3979"},
  {id:"linjer",company:"Linjer",subject:"We've received your application",date:"2026-06-07",status:"仅申请确认",tone:"info",summary:"系统确认已收到申请；表示如果匹配会在两周内联系。目前已超过其预计回复时间。",translation:"你好，陈若竹：\n\n感谢你的申请。如果我们认为你与岗位有较好的匹配潜力，团队将在未来两周内与你联系。\n\nLinjer 团队",original:"Hello 陈若竹,\n\nThank you for applying. Our team will be in touch within the next 2 weeks in case we see a good potential fit.\n\n- The Linjer Team",next:"这不是面试邀请。因已超过两周且没有后续邮件，可标记为“暂无进展”，无需继续等待。",gmailUrl:"https://mail.google.com/mail/#all/19ea241fef11211d"}
];
...
export default function Home(){
  ...
  const loadBackend=async()=>{try{const [c,a]=await Promise.all([fetch('/api/connections',{cache:'no-store'}),fetch('/api/applications',{cache:'no-store'})]);if(c.ok)setConnections((await c.json()).channels||[]);if(a.ok){const apps=(await a.json()).applications||[];setApplied(apps.map((x:any)=>x.gigId));}}catch{}};
  ...
  return <main className="app-shell">
    ...
    <nav className="bottom-nav" aria-label="主导航">{[['机会','home'],['进度','brief'],['连接','check'],['回复','mail'],['我的','user']].map(([label,icon])=><button key={label} className={activeTab===label?'active':''} onClick={()=>setActiveTab(label)}><Icon name={icon}/><span>{label}</span>{label==='进度'&&applied.length>0&&<i className="badge">{applied.length}</i>}{label==='回复'&&applicationReplies.filter(r=>r.tone==='action').length>0&&<i className="badge">{applicationReplies.filter(r=>r.tone==='action').length}</i>}</button>)}</nav>
