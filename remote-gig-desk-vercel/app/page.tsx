    ...
    <section className="gig-list">
      {activeTab==='进度'?<div className="application-list">{applications.length?applications.map(app=>{
        ...
        <section className="browser-agent-panel">
          ...
          {pairingToken&&<div className="pairing-token"><b>一次性配对令牌</b><code>{pairingToken}</code><button onClick={()=>navigator.clipboard?.writeText(pairingToken)}>复制令牌</button><p>安装浏览器执行器后粘贴此令牌，并填写 Hacker News 用户名 vesperChen。令牌仅显示这一次。</p></div>}
