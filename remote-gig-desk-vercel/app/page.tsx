...
export default function Home(){
  ...
  const loadBackend=async()=>{try{const [c,a,r]=await Promise.all([fetch('/api/connections',{cache:'no-store'}),fetch('/api/applications',{cache:'no-store'}),fetch('/api/replies',{cache:'no-store'})]);if(c.ok)setConnections((await c.json()).channels||[]);if(a.ok){const apps=(await a.json()).applications||[];setApplied(apps.map((x:any)=>x.gigId));}if(r.ok)setReplies((await r.json()).replies||[]);}catch{}};
