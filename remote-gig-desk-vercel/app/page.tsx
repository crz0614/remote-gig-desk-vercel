...
type ApplicationRecord = { id:string; gigId:string; title:string; source:string; sourceUrl:string; status:string; deliveryChannel:string; proposedRate:string; createdAt:string; updatedAt:string; destination?:string; lastError?:string };
  ...
  return <main className="app-shell">
    ...
    <section className="gig-list">
      {activeTab==='进度'?<div className="application-list">{applications.length?applications.map(app=>{
        ...
        return <article className="gig-card application-record" key={app.id}><div className="card-top"><div className="source-icon">{app.source?.[0]||'申'}</div><div className="source"><b>{app.source}</b><span>{new Date(Number(app.updatedAt)||app.updatedAt).toLocaleString('zh-CN')}</span></div><span className={'reply-status '+(app.status==='submitted'?'info':app.status==='submission_failed'?'warning':'action')}>{label}</span></div><h3>{app.title}</h3><div className="facts"><div><small>申请报价</small><b>{app.proposedRate||'待确认'}</b></div><div><small>投递渠道</small><b>{app.deliveryChannel}</b></div></div><p className="application-note">任务编号：{app.id}</p>{app.lastError&&<p className="application-error">失败原因：{app.lastError}</p>}<a className="secondary-link" href={app.sourceUrl} target="_blank" rel="noreferrer">查看甲方原始页面 ↗</a><div className="progress-actions">{app.status!=='submitted'&&app.status!=='cancelled'&&<button disabled={queueing} onClick={()=>updateApplicationStatus(app.id,'mark_submitted')}>我已在原平台提交</button>}{['detecting_destination','manual_submission_required'].includes(app.status)&&<button disabled={queueing} onClick={()=>updateApplicationStatus(app.id,'needs_verification')}>需要登录或验证码</button>}{app.status==='submission_failed'&&<button disabled={queueing} onClick={()=>retryApplication(app.id)}>重新投递</button>}{app.status==='verification_required'&&<button disabled={queueing} onClick={()=>retryApplication(app.id)}>验证完成，继续投递</button>}{app.status!=='submitted'&&app.status!=='cancelled'&&<button className="danger-action" disabled={queueing} onClick={()=>updateApplicationStatus(app.id,'cancel')}>取消任务</button>}</div></article>;
