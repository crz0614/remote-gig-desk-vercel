...
type Reply = { id:string; company:string; subject:string; date:string; status:string; tone:"action"|"info"|"warning"; summary:string; translation:string; original:string; next:string; gmailUrl:string; applicationId?:string; applicationTitle?:string };
...
type AuthenticatedSite = { platformKey:string; name:string; status:string; sessionType:string; accountLabel?:string; siteUrl?:string; verifiedAt?:string|number; lastCheckedAt?:string|number; updatedAt?:string|number; expiresAt?:string|number; note:string };
type BrowserAgent = { id:string; name:string; status:string; lastSeenAt?:string|number; createdAt?:string|number; updatedAt?:string|number };
type ApplicationEvent = { id:string; eventType:string; status:string; message:string; evidenceId?:string; evidenceUrl?:string; createdAt:string };
type ApplicationRecord = { id:string; gigId:string; title:string; source:string; sourceUrl:string; applicationUrl?:string; status:string; deliveryChannel:string; proposedRate:string; createdAt:string; updatedAt:string; destination?:string; lastError?:string; platformKey?:string; deliveryState?:string; receiptId?:string; receiptUrl?:string; deliveredAt?:string; materials?:{language?:string;matchedSkills?:string[];resumeHighlights?:string[];coverLetter?:string;workMode?:string}; events?:ApplicationEvent[] };
...
function englishQuote(gig:Gig){
  return (gig.budget==='预算面议'?'Rate to be confirmed with the client':gig.budget)
...
function createApplicationPack(gig:Gig):ApplicationPack{
  ...
  if(chinese){
    bullets.push(`与该项目直接相关的技能：${matched.slice(0,4).join("、")||"待从私有资料中确认"}。提交前需要从私有资料中选择可核验的项目证据。`);
  }else{
    bullets.push(`Directly relevant skills: ${matched.slice(0,4).join(", ")||"to be confirmed from the private profile"}. Select a verifiable project example from the private profile before submission.`);
  }
  const quote=chinese?(gig.budget==='预算面议'?'待与甲方确认':gig.budget):englishQuote(gig);
  const coverLetter=chinese?`您好，\n\n我希望申请“${gig.title}”项目。与该需求匹配的技能包括：${matched.slice(0,3).join('、')||'待确认'}。相关项目证据将从私有资料中选择并在提交前核验。\n\n期望报价为 ${quote}，最终价格可根据确认后的范围与交付物协商。\n\n此致\n申请人`:`Hello,\n\nI am applying for ${gig.title}. The relevant skills identified for this role are ${matched.slice(0,3).join(', ')||'to be confirmed'}. A verified project example will be selected from the private profile before submission.\n\nMy expected rate is ${quote}; the final quote can be refined once the scope and deliverables are confirmed.\n\nBest regards,\nApplicant`;
...
export default function Home(){
  ...
  const [connections,setConnections]=useState<ChannelConnection[]>([]); const [authenticatedSites,setAuthenticatedSites]=useState<AuthenticatedSite[]>([]); const [browserAgents,setBrowserAgents]=useState<BrowserAgent[]>([]); const [pairingToken,setPairingToken]=useState(""); const [pairing,setPairing]=useState(false); const [queueing,setQueueing]=useState(false); const [queueNotice,setQueueNotice]=useState<{id:string;status:string;channel:string}|null>(null);
  ...
  const updateApplicationStatus=async(id:string,action:"needs_verification"|"verify_platform"|"cancel")=>{setQueueing(true);try{const r=await fetch('/api/applications/status',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id,action})});if(!r.ok)throw new Error();await loadBackend();}catch{setMailNotice('状态更新失败，请重试');}finally{setQueueing(false);}};
  const retryApplication=async(id:string)=>{setQueueing(true);try{const r=await fetch('/api/applications/retry',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id})});const result=await r.json();if(!r.ok&&result.status!=='verification_required')throw new Error();await loadBackend();}catch{setMailNotice('重新投递失败，请在进度中查看失败原因');}finally{setQueueing(false);}};
  const verifyPlatformAndContinue=async(app:ApplicationRecord)=>{setQueueing(true);setMailNotice('');try{const verified=await fetch('/api/applications/status',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:app.id,action:'verify_platform'})});if(!verified.ok)throw new Error('平台验证状态保存失败');const retried=await fetch('/api/applications/retry',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({platformKey:app.platformKey})});const result=await retried.json();if(!retried.ok)throw new Error(result.error||'同平台任务继续失败');setMailNotice(`已继续处理同平台 ${result.processed||0} 个任务；平台接口确认接收 ${result.accepted||0} 个`);await loadBackend();}catch(error){setMailNotice(error instanceof Error?error.message:'同平台任务继续失败');}finally{setQueueing(false);}};
  const gigs=data?.gigs??[];
  const availableGigs=useMemo(()=>filterAvailableGigs(gigs,applied),[gigs,applied]);
  const visible=useMemo(()=>{let v=activeTab==='收藏'?gigs.filter(g=>saved.includes(g.id)):activeTab==='进度'?gigs.filter(g=>applied.includes(g.id)):activeTab==='机会'?availableGigs:gigs;if(query)v=v.filter(g=>`${g.title} ${g.summary} ${g.skills.join(' ')}`.toLowerCase().includes(query.toLowerCase()));if(filter==='最新')v=[...v].sort((a,b)=>+new Date(b.publishedAt)-+new Date(a.publishedAt));if(filter==='低竞争')v=v.filter(g=>g.competition==='低');if(filter==='高预算')v=v.filter(g=>g.budget!=='预算面议');return v;},[gigs,availableGigs,activeTab,saved,applied,query,filter]);
  ...
  const translateGig=async(gig:Gig)=>{setTranslating(true);setTranslation('');setTranslatedTitle('');const cacheKey=`gig-zh-v5-${gig.id}`;try{const cached=localStorage.getItem(cacheKey);if(cached){const parsed=JSON.parse(cached);setTranslation(parsed.text||'');setTranslatedTitle(parsed.title||'');return;}const translate=async(text:string)=>{if(!text.trim())return '';const chinese=(text.match(/[\u4e00-\u9fff]/g)||[]).length;if(chinese>Math.max(12,text.length*.15))return text;const response=await fetch('/api/translate',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({text})});if(!response.ok)throw new Error('translation_failed');const result=await response.json();return String(result.translated||'').trim();};const [title,text]=await Promise.all([translate(gig.title),translate(gig.fullText||gig.summary)]);if(!text)throw new Error('translation_empty');setTranslatedTitle(title&&title!==gig.title?title:'');setTranslation(text);localStorage.setItem(cacheKey,JSON.stringify({title:title&&title!==gig.title?title:'',text}));}catch{setTranslation('');setTranslatedTitle('');}finally{setTranslating(false);}};
  const openGig=async(gig:Gig)=>{setSelected(gig);setShowOriginal(false);await translateGig(gig);};
  const copyApplication=(gig:Gig)=>{const pricing=gig.budget==='预算面议'?'甲方未公开预算，申请前必须确认报价，不得自行编造金额。':`甲方原文标注金额：${gig.budget}；提交前仍需核对金额含义、币种和计价周期。`;navigator.clipboard?.writeText(`请帮我申请这个项目：${translatedTitle||gig.title}\n来源：${gig.source}\n原始链接：${gig.sourceUrl}\n${pricing}\n远程：${gig.remote}`);};
  ...
  return <main className="app-shell">
    ...
    {activeTab==='机会'&&<>
      ...
      <section className="hero-card"><div className="hero-copy"><span className="live-pill"><i/>真实项目 · 原始链接</span><h2>当前找到 <strong>{availableGigs.length}</strong> 个<br/>未申请机会</h2><p>已建立申请任务的岗位会自动移至“进度”</p></div><div className="score-ring"><span>最高</span><b>{availableGigs[0]?.match??'—'}</b><small>匹配度</small></div></section>
      <section className="quick-stats" aria-label="项目概览"><div><b>{availableGigs.length}</b><span>未申请</span></div><div><b>{availableGigs.filter(g=>g.competition==='低').length}</b><span>低竞争</span></div><div><b>{applications.length}</b><span>申请任务</span></div></section>
    ...
    <section className="gig-list">
      {activeTab==='进度'?<div className="application-list">{applications.length?applications.map(app=>{
        const labels:Record<string,string>={submitted:'平台已接收',response_received:'已收到对方回复',queued_for_browser:'浏览器执行队列中',browser_in_progress:'浏览器正在处理',form_ready:'表单已填写',detecting_destination:'正在识别投递入口',awaiting_github_authorization:'等待 GitHub 授权',submission_failed:'投递失败',verification_required:'同平台等待一次验证',cancelled:'已取消'};
        const label=labels[app.status]||app.status;
        const proof=app.deliveryState==='recipient_replied'?{title:'对方已回复',text:'已收到对方邮件回复，申请已进入沟通流程。',tone:'received'}:app.deliveryState==='platform_accepted'?{title:'平台接口已确认接收',text:'平台返回了可核验的提交回执；这不等同于对方已阅读。',tone:'accepted'}:app.deliveryState==='session_reused'?{title:'平台会话已复用',text:'任务已进入浏览器执行队列；同平台后续任务无需重复登录。',tone:'session'}:app.deliveryState==='form_ready'?{title:'申请表单已填写',text:'浏览器已完成已知字段填写，等待受保护确认或提交回执。',tone:'session'}:null;
        return <article className="gig-card application-record" key={app.id}><div className="card-top"><div className="source-icon">{app.source?.[0]||'申'}</div><div className="source"><b>{app.source}</b><span>{new Date(Number(app.updatedAt)||app.updatedAt).toLocaleString('zh-CN')}</span></div><span className={'reply-status '+(app.status==='submitted'||app.status==='response_received'?'info':app.status==='submission_failed'?'warning':'action')}>{label}</span></div><h3>{app.title}</h3><div className="facts"><div><small>申请报价</small><b>{app.proposedRate||'待确认'}</b></div><div><small>投递渠道</small><b>{app.deliveryChannel}</b></div></div><p className="application-note">任务编号：{app.id}</p>{app.materials?.matchedSkills?.length&&<div className="profile-skills">{app.materials.matchedSkills.map(skill=><span key={skill}>{skill}</span>)}</div>}{app.platformKey&&<p className="platform-session">平台队列：{app.platformKey} · 同平台只验证一次</p>}{app.lastError&&<p className="application-error">失败原因：{app.lastError}</p>}{proof&&<div className={'task-proof '+proof.tone}><b>{proof.title}</b><p>{proof.text}</p>{app.receiptUrl?<a href={app.receiptUrl} target="_blank" rel="noreferrer">查看平台回执 ↗</a>:app.receiptId?<small>回执编号：{app.receiptId}</small>:null}</div>}<h4 className="timeline-title">任务时间线</h4><ol className="task-timeline">{(app.events||[]).length?(app.events||[]).map(event=><li key={event.id}><i/><div><time>{new Date(Number(event.createdAt)||event.createdAt).toLocaleString('zh-CN')}</time><b>{event.message}</b><small>{labels[event.status]||event.status}</small>{event.evidenceUrl?<a href={event.evidenceUrl} target="_blank" rel="noreferrer">查看证据 ↗</a>:event.evidenceId?<span>证据编号：{event.evidenceId}</span>:null}</div></li>):<li><i/><div><b>任务已建立，等待下一步记录</b></div></li>}</ol><a className="secondary-link" href={app.sourceUrl} target="_blank" rel="noreferrer">查看甲方原始页面 ↗</a>{app.applicationUrl&&app.applicationUrl!==app.sourceUrl&&<a className="secondary-link" href={app.applicationUrl} target="_blank" rel="noreferrer">查看最终申请入口 ↗</a>}<div className="progress-actions">{app.status==='detecting_destination'&&<button disabled={queueing} onClick={()=>updateApplicationStatus(app.id,'needs_verification')}>此平台需要登录或验证码</button>}{app.status==='submission_failed'&&<button disabled={queueing} onClick={()=>retryApplication(app.id)}>重新处理此任务</button>}{app.status==='verification_required'&&<button disabled={queueing} onClick={()=>verifyPlatformAndContinue(app)}>本平台验证完成，继续全部任务</button>}{!['submitted','response_received','cancelled'].includes(app.status)&&<button className="danger-action" disabled={queueing} onClick={()=>updateApplicationStatus(app.id,'cancel')}>取消任务</button>}</div></article>;
      }):<div className="empty-card"><h3>还没有申请任务</h3><p>在机会页多选岗位后，点击“申请全部已选”。</p></div>}</div>:activeTab==='回复'?<div className="reply-list">{replies.map(reply=><button className="reply-card" key={reply.id} onClick={()=>setSelectedReply(reply)}><div className="reply-card-top"><span className={`reply-status ${reply.tone}`}>{reply.status}</span><time>{reply.date}</time></div><h3>{reply.subject}</h3><b>{reply.company}</b>{reply.applicationTitle&&<p className="reply-linked">对应申请：{reply.applicationTitle}</p>}<p>{reply.summary}</p><span className="reply-open">查看中文与原文 <Icon name="arrow"/></span></button>)}</div>:activeTab==='连接'?<div className="connection-list">
        <section className="browser-agent-panel">
          <div className="auth-overview-head"><div><small>本机 Chrome 执行器</small><h3>{browserAgents.filter(agent=>agent.status==='online').length} 个在线</h3></div><span>{browserAgents.some(agent=>agent.status==='online')?'可复用本机登录':'等待配对'}</span></div>
          <p>任务在你已经登录的平台页面中执行。Cookie、密码和验证码不会上传到工作台；服务器只保存执行器在线状态和平台认证结果。</p>
          <div className="agent-list">{browserAgents.map(agent=><article className="agent-row" key={agent.id}><div><b>{agent.name}</b><span className={agent.status==='online'?'agent-online':'agent-offline'}>{agent.status==='online'?'在线':'离线'}</span></div>{agent.lastSeenAt&&<small>最近心跳：{new Date(Number(agent.lastSeenAt)).toLocaleString('zh-CN')}</small>}</article>)}</div>
        ...
        <section className="auth-overview">
          ...
          <p>同一平台的岗位共用一次登录或 OAuth 授权。只要网站 Cookie / 令牌仍有效，后续申请不必重复登录；过期、退出或撤销后才会要求重新验证一次。</p>
          <div className="auth-site-list">{authenticatedSites.length?authenticatedSites.map(site=>{
            const active=['connected','verified'].includes(site.status);
            const checked=site.lastCheckedAt||site.updatedAt||site.verifiedAt;
            return <article className="auth-site" key={site.platformKey}>
              <div className="auth-site-title"><span className={active?'auth-state active':'auth-state needs-check'}>{active?'✓':'!'}</span><div><h4>{site.name}</h4><b>{site.accountLabel||'已保存平台认证记录'}</b></div><em>{site.sessionType==='oauth'?'OAuth':'浏览器会话'}</em></div>
              <p>{site.note}</p>
              <footer><span>{active?'可供同平台任务复用':'下次申请前需验证'}</span>{checked&&<time>最近核验：{new Date(Number(checked)).toLocaleString('zh-CN',{month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit'})}</time>}</footer>
            </article>;
      ...
      visible.length?visible.map((gig,index)=><article className={`gig-card ${index===0&&activeTab==='机会'?'featured':''}`} key={gig.id}>
        ...
        <div className="facts"><div><small>{gig.budget==='预算面议'?'预算状态':'原文金额'}</small><b>{gig.budget==='预算面议'?'甲方未公开':gig.budget}</b></div><div><small>竞争</small><b className="low"><i/>{gig.competition}</b></div><div className="match"><small>匹配度</small><b>{gig.match}%</b></div></div>
    ...
    {selectedReply&&<div className="modal-backdrop" onClick={()=>setSelectedReply(null)}><section className="detail-sheet reply-detail" onClick={e=>e.stopPropagation()}><div className="grabber"/><button className="close" onClick={()=>setSelectedReply(null)}>×</button><span className={`reply-status ${selectedReply.tone}`}>{selectedReply.status}</span><h2>{selectedReply.subject}</h2><p className="reply-company">{selectedReply.company} · {selectedReply.date}</p>{selectedReply.applicationTitle&&<p className="application-note">对应申请：{selectedReply.applicationTitle}</p>}<h3>中文整理</h3><div className="translation-box"><p>{selectedReply.translation}</p></div><h3>你需要做什么</h3><p className="application-note">{selectedReply.next}</p><h3>邮件原文</h3><div className="original-mail">{selectedReply.original}</div><a className="apply-button" href={selectedReply.gmailUrl} target="_blank" rel="noreferrer">在 Gmail 中打开 ↗</a></section></div>}
    {selected&&<div className="modal-backdrop" onClick={()=>setSelected(null)}><section className="detail-sheet" onClick={e=>e.stopPropagation()}><div className="grabber"/><button className="close" onClick={()=>setSelected(null)}>×</button><span className="detail-source">{selected.source} · {age(selected.publishedAt)}</span><h2>{translatedTitle||selected.title}</h2>{translatedTitle&&<p className="original-title">英文标题：{selected.title}</p>}<div className="detail-grid"><div><small>{selected.budget==='预算面议'?'预算状态':'原文金额'}</small><b>{selected.budget==='预算面议'?'甲方未公开':selected.budget}</b></div><div><small>工作方式</small><b>{selected.remote}</b></div></div><h3>甲方需求（逐岗位中文翻译）</h3><div className="translation-box">{translating?<span className="translate-loading">正在翻译该岗位原文…</span>:translation?<p>{translation}</p>:<div className="translate-failed"><p>本次逐岗位翻译失败，未使用通用模板替代。</p><button onClick={()=>translateGig(selected)}>重新翻译</button></div>}</div><button className="original-toggle" onClick={()=>setShowOriginal(v=>!v)}>{showOriginal?'收起英文原文':'查看英文原文'}</button>{showOriginal&&<p className="original-text">{selected.fullText||selected.summary}</p>}<h3>原文识别技能</h3><div className="tags">{(selected.skills.length?selected.skills:['原文未识别出明确技能']).map(s=><span key={s}>{s}</span>)}</div><h3>申请方式</h3><p className="application-note">{selected.application}</p><button className="apply-button" onClick={()=>startApplication(selected)}>一键申请此岗位</button><a className="secondary-link" href={selected.sourceUrl} target="_blank" rel="noreferrer">查看原始需求 ↗</a></section></div>}
