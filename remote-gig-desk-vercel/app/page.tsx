...
type ApplicationRecord = { id:string; gigId:string; title:string; source:string; sourceUrl:string; status:string; deliveryChannel:string; proposedRate:string; createdAt:string; updatedAt:string; destination?:string; lastError?:string; platformKey?:string; deliveryState?:string; receiptId?:string; receiptUrl?:string; deliveredAt?:string; events?:ApplicationEvent[] };
...
export default function Home(){
  ...
  const updateApplicationStatus=async(id:string,action:"mark_submitted"|"needs_verification"|"verify_platform"|"cancel")=>{setQueueing(true);try{const r=await fetch('/api/applications/status',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id,action})});if(!r.ok)throw new Error();await loadBackend();}catch{setMailNotice('状态更新失败，请重试');}finally{setQueueing(false);}};
  ...
  return <main className="app-shell">
    ...
    <section className="gig-list">
      {activeTab==='进度'?<div className="application-list">{applications.length?applications.map(app=>{
        const labels:Record<string,string>={submitted:'平台已接收',manual_confirmed:'人工确认已提交',response_received:'已收到对方回复',manual_submission_required:'等待在原平台提交',detecting_destination:'正在识别投递入口',awaiting_github_authorization:'等待 GitHub 授权',submission_failed:'投递失败',verification_required:'同平台等待一次验证',cancelled:'已取消'};
        const label=labels[app.status]||app.status;
        const proof=app.deliveryState==='recipient_replied'?{title:'对方已回复',text:'已收到对方邮件回复，申请已进入沟通流程。',tone:'received'}:app.deliveryState==='platform_accepted'?{title:'平台接口已确认接收',text:'平台返回了可核验的提交回执；这不等同于对方已阅读。',tone:'accepted'}:app.deliveryState==='user_confirmed'?{title:'仅由你确认已提交',text:'目前没有平台接口回执，不能标记为对方已收到。',tone:'manual'}:app.deliveryState==='platform_session_verified'?{title:'平台登录已验证',text:'同平台后续任务复用本次会话，无需重复登录或验证码。',tone:'session'}:null;
        return <article className="gig-card application-record" key={app.id}><div className="card-top"><div className="source-icon">{app.source?.[0]||'申'}</div><div className="source"><b>{app.source}</b><span>{new Date(Number(app.updatedAt)||app.updatedAt).toLocaleString('zh-CN')}</span></div><span className={'reply-status '+(app.status==='submitted'||app.status==='response_received'?'info':app.status==='submission_failed'?'warning':'action')}>{label}</span></div><h3>{app.title}</h3><div className="facts"><div><small>申请报价</small><b>{app.proposedRate||'待确认'}</b></div><div><small>投递渠道</small><b>{app.deliveryChannel}</b></div></div><p className="application-note">任务编号：{app.id}</p>{app.platformKey&&<p className="platform-session">平台队列：{app.platformKey} · 同平台只验证一次</p>}{app.lastError&&<p className="application-error">失败原因：{app.lastError}</p>}{proof&&<div className={'task-proof '+proof.tone}><b>{proof.title}</b><p>{proof.text}</p>{app.receiptUrl?<a href={app.receiptUrl} target="_blank" rel="noreferrer">查看平台回执 ↗</a>:app.receiptId?<small>回执编号：{app.receiptId}</small>:null}</div>}<h4 className="timeline-title">任务时间线</h4><ol className="task-timeline">{(app.events||[]).length?(app.events||[]).map(event=><li key={event.id}><i/><div><time>{new Date(Number(event.createdAt)||event.createdAt).toLocaleString('zh-CN')}</time><b>{event.message}</b><small>{labels[event.status]||event.status}</small>{event.evidenceUrl?<a href={event.evidenceUrl} target="_blank" rel="noreferrer">查看证据 ↗</a>:event.evidenceId?<span>证据编号：{event.evidenceId}</span>:null}</div></li>):<li><i/><div><b>任务已建立，等待下一步记录</b></div></li>}</ol><a className="secondary-link" href={app.sourceUrl} target="_blank" rel="noreferrer">查看甲方原始页面 ↗</a><div className="progress-actions">{app.status==='manual_submission_required'&&<button disabled={queueing} onClick={()=>updateApplicationStatus(app.id,'mark_submitted')}>我已在原平台完成提交</button>}{['detecting_destination','manual_submission_required'].includes(app.status)&&<button disabled={queueing} onClick={()=>updateApplicationStatus(app.id,'needs_verification')}>此平台需要登录或验证码</button>}{app.status==='submission_failed'&&<button disabled={queueing} onClick={()=>retryApplication(app.id)}>重新处理此任务</button>}{app.status==='verification_required'&&<button disabled={queueing} onClick={()=>verifyPlatformAndContinue(app)}>本平台验证完成，继续全部任务</button>}{!['submitted','manual_confirmed','response_received','cancelled'].includes(app.status)&&<button className="danger-action" disabled={queueing} onClick={()=>updateApplicationStatus(app.id,'cancel')}>取消任务</button>}</div></article>;
