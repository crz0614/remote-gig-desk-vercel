...
- Aggregates opportunities from GitHub, Hacker News, RemoteOK, Remotive, Jobicy, Arbeitnow, Reddit, YC/company career pages, and supported ATS sources.
...
- Queues Greenhouse, Lever, Ashby, and Workable forms for cloud or paired-browser execution.
- Uploads real attachments and records submission only after validating an official confirmation page or API receipt.
- Moves CAPTCHA, MFA, identity, legal-consent, and missing-required-field checkpoints to a human takeover queue.
...
- `browser-agent/` — paired Chrome extension
